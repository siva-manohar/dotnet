pipeline {
  ...
  stages {
    ...
    stage('Deploy docker'){
      steps {
		sh '''
			  ssh -i /var/lib/jenkins/.ssh/publickey.pem -o StrictHostKeyChecking=no ubuntu@ec2-46-137-235-134.ap-southeast-1.compute.amazonaws.com 'bash -s' < ./deploy.sh \${BUILD_NUMBER}
			  '''	 
